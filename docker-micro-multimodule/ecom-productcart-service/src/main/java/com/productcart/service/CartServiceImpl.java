...
public class CartServiceImpl implements ICartService {
	...
	public Type addToCart(int userId, int productId, int quantity) {
		// check if a cart exists for a specific userId
		// if so get the cart
		Cart cart = getCartByUserId(userId);
		cart.setUserId(userId);

		// get the product from product-info service
	...
	private Cart getCartByUserId(int userId) {
		Cart cart = cartRepository.findById(userId)
				...
				.orElseGet(() -> {
					...
					// once the cart is created. we have to set the userid and cart items
					// cart items should not be null so created arraylist
	...
	private Product getByProductId(int productId) {
		// get the product from product-info
		// when we click the product in cart it should display whole info about product
		// so we are doing
		// communication bw services using @Feign client
		// add feignclient dependency in pom
	...
	public void removeFromCart(int userId, int productId) {
		Cart cart = cartRepository.findByUserId(userId).orElseThrow(() -> new CartNotFoundException("invalid userId"));
		boolean removed = cart.getCartItems().removeIf(cartItem -> cartItem.getProductId() == productId);
